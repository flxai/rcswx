...
from .. import layers
from ..genotype import Operation
...
sequential_module = Operation(
    ...
    inherit=[inherit_first_child, inherit_other_child],
    give_back=[give_back_default, give_back_default],
    type="nonterminal",
...
def build_sequential_module_k(node, k):
    ...
    def infer(node):
        ...
        print(
            f"Inferred {node} with input params {node.input_params} and output params {node.output_params}"
        )
    ...
    return layers.SequentialModule4(fns=modules)
...
branching_module_2 = Operation(
    ...
    inherit=[inherit_first_child, inherit_branching, inherit_branching, inherit_aggregation],
    give_back=[give_back_default, give_back_default, give_back_default, give_back_default],
    type="nonterminal",
    child_levels=["branching_fn_2", "module", "module", "aggregation_fn_2"],
)
...
def build_branching_module_4(node):
    return layers.BranchingModule(
        ...
        inner_fn=[
            node.children[1].build(node.children[1])
            for _ in range(node.children[1].input_params["branching_factor"])
        ],
...
branching_module_4 = Operation(
    ...
    inherit=[inherit_first_child, inherit_branching, inherit_aggregation],
    give_back=[give_back_default, give_back_default, give_back_default],
    type="nonterminal",
    child_levels=["branching_fn_4", "module", "aggregation_fn_4"],
)
...
branching_module_8 = Operation(
    ...
    give_back=[give_back_default, give_back_default, give_back_default],
    type="nonterminal",
    child_levels=["branching_fn_8", "module", "aggregation_fn_8"],
)
...
routing_module = Operation(
    ...
    inherit=[inherit_first_child, inherit_other_child, inherit_other_child],
    give_back=[give_back_default, give_back_default, give_back_default],
    type="nonterminal",
    child_levels=["prerouting_fn", "module", "postrouting_fn"],
)


def build_computation_module(node):
    return layers.ComputationModule(computation_fn=node.children[0].build(node.children[0]))
...
def build_group(branching_factor, dim, node):
    return layers.GroupDim(
        splits=branching_factor, dim=dim, dim_total=node.input_params["shape"][dim]
    )
...
def valid_group(branching_factor, dim, node):
    return (
        len(node.input_params["shape"]) > dim
        and node.input_params["shape"][dim] > 0
        and (node.input_params["shape"][dim] >= branching_factor)
        and (node.input_params["shape"][dim] % branching_factor == 0)
    )
...
def valid_add(node):
    return node.input_params["other_shape"] != None and torch.equal(
        torch.tensor(node.input_params["shape"]), torch.tensor(node.input_params["other_shape"])
    )
...
def valid_cat(dim, node):
    return (
        len(node.input_params["shape"]) > dim
        and len(node.input_params["other_shape"]) > dim
        and (node.input_params["other_shape"] != None)
        and torch.equal(
            torch.tensor(node.input_params["shape"][:dim] + node.input_params["shape"][dim + 1 :]),
            torch.tensor(
                node.input_params["other_shape"][:dim] + node.input_params["other_shape"][dim + 1 :]
            ),
        )
    )
...
def infer_dot_product(node):
    shape = torch.Size(
        list(node.input_params["shape"][:-1]) + [node.input_params["other_shape"][-1]]
    )
    return {
        "shape": shape,
        "other_shape": node.parent.input_params["other_shape"],
...
def valid_dot_product(node):
    return (
        node.input_params["other_shape"] != None
        and node.input_params["shape"][-1] == node.input_params["other_shape"][-2]
        and torch.equal(
...
def valid_im2col(kernel_size, node):
    return (
        len(node.input_params["shape"]) == 4
        and node.input_params["mode"] == "im"
        and (node.input_params["shape"][-2] >= kernel_size)
        and (node.input_params["shape"][-1] >= kernel_size)
    )
...
def valid_col2im(node):
    return (
        len(node.input_params["shape"]) == 3
        and node.input_params["mode"] == "col"
        and (node.input_params["last_im_shape"] != None)
        and (
            node.input_params["shape"][1]
            == torch.prod(torch.tensor(node.input_params["last_im_shape"]))
        )
    )
...
def infer_linear(dim, node):
    return {
        "shape": torch.Size(list(node.input_params["shape"][:-1]) + [dim]),
        "other_shape": node.input_params["other_shape"],
        "mode": node.input_params["mode"],
        "other_mode": node.input_params["other_mode"],
        "branching_factor": node.input_params["branching_factor"],
...
def infer_linear_x(dim_factor, node):
    out_dim = int(node.input_params["shape"][-1] * dim_factor)
    return {
        "shape": torch.Size(list(node.input_params["shape"][:-1]) + [out_dim]),
        "other_shape": node.input_params["other_shape"],
        "mode": node.input_params["mode"],
        "other_mode": node.input_params["other_mode"],
        "branching_factor": node.input_params["branching_factor"],
        "last_im_shape": node.input_params["last_im_shape"],
        "num_params": node.input_params["shape"][-1] * out_dim + out_dim,
    }
...
modules_without_computation_module = {
    ...
    "probs": [0.333, 0.111, 0.111, 0.111, 0.333],
}
deep_modules_without_computation_module = {
    ...
    "probs": [0.111, 0.111, 0.111, 0.111, 0.111, 0.111, 0.333],
}
modules = lambda prob: {
    "options": [
        sequential_module,
    ...
    "probs": [(1 - prob) / 3, (1 - prob) / 9, (1 - prob) / 9, (1 - prob) / 9, (1 - prob) / 3, prob],
}
...
branching_fns_2 = {
    "options": [clone(2), group(2, 1), group(2, 2), group(2, 3)],
    "probs": [0.25, 0.25, 0.25, 0.25],
}
branching_fns_4 = {
    "options": [clone(4), group(4, 1), group(4, 2), group(4, 3)],
    "probs": [0.25, 0.25, 0.25, 0.25],
}
branching_fns_8 = {
    "options": [clone(8), group(8, 1), group(8, 2), group(8, 3)],
    "probs": [0.25, 0.25, 0.25, 0.25],
}
aggregation_fns_2 = {
    "options": [
...
aggregation_fns_4 = {
    "options": [add(4), cat(4, 1), cat(4, 2), cat(4, 3)],
    "probs": [0.25, 0.25, 0.25, 0.25],
}
aggregation_fns_8 = {
    "options": [add(8), cat(8, 1), cat(8, 2), cat(8, 3)],
    "probs": [0.25, 0.25, 0.25, 0.25],
}
aggregation_fns_with_broadcast_2 = {
    ...
    "probs": [0.143, 0.143, 0.143, 0.143, 0.143, 0.143, 0.143],
}
prerouting_fns = {
    "options": [
        identity,
...
quick_deep_broadcast_grammar = {
    "network": {
        "options": [sequential_module, sequential_module_4, sequential_module_8],
